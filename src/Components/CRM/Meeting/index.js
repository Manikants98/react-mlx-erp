...
const Meetings = () => {
  ...
  const isAddPermit = usePermission("add_leads_meeting")
  const isDeletePermit = usePermission("delete_leads_meeting")
  const isViewPermit = usePermission("view_leads_meeting")

  const { data, isLoading, refetch } = useQuery(["meetings", search, page, value], () =>
            ...
            <TableBody>
              ...
              {meetings?.map((meeting, index) => (
                <TableRow key={meeting.id || index}>
                  ...
                  {value === "Open" && (
                    <TableCell>
                      <Reschedule meeting={meeting} refetch={refetch} />
                    </TableCell>
                  )}
                  <TableCell>
