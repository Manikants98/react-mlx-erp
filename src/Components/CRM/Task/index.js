...
import { Checkbox, Table, TableBody, MenuItem, TableContainer, TableHead, TableRow } from "@mui/material"
...
const Task = () => {
  ...
  const isAddPermit = usePermission("CRM", "add_leads_task")
  const isUpdatePermit = usePermission("CRM", "update_leads_task")
  const isDeletePermit = usePermission("CRM", "delete_leads_task")
  const isViewPermit = usePermission("CRM", "view_leads_task")

  const {
    data: tasksList,
    isLoading,
    refetch,
  } = useQuery(["tasksList", search, page, priority, status], () => tasksFn({ search, page, priority, status }))
      ...
      <GlassDiv className="flex flex-col !p-0">
        <div className="flex justify-between p-2">
          ...
          <span className="flex items-center gap-3">
            <DeleteButton
              isDelete={isDelete}
              onConfirm={() => deleteTask({ campaign_task_ids: selectedIds })}
                  ...
                  return (
                    <TableRow key={task.id} className="hover:bg-white hover:bg-opacity-20">
                      ...
                      <TableCell>
                        <div className="flex items-center justify-between px-3">
                          {task.status || "-"}
                          <TaskStatus task_status={task.status} task_id={task.id} />
                        </div>
                      </TableCell>
                      <TableCell>
                        <CustomIconButton
