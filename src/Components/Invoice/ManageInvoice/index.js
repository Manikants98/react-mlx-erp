...
const ManageInvoice = () => {
  ...
  useEffect(() => {
    if (!isUpdate) {
      if (payment_type && formik.values.invoice_type === "DEAL") {
        let total_amount = 0
        total_amount = payment_type === "Advance payment" ? deals?.advance_amount : deals?.after_campaign_amount
  ...
  useEffect(() => {
    if (!isUpdate) {
      ...
      if (formik.values.invoice_type === "DEAL")
        if (deals.advance_payment && deals?.after_campaign_payment) {
          console.log("Choose Payment Type")
        } else {
          if (deals.advance_payment) {
            payment_type = "Advance payment"
          } else if (deals?.after_campaign_payment) {
    ...
    <GlassDiv className="w-full !p-0">
      ...
      <form onSubmit={formik.handleSubmit} className="grid grid-cols-1 gap-3 p-3">
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-3 md:grid-cols-2">
          ...
          {formik.values.invoice_type === "DEAL" && deals?.advance_payment && deals?.after_campaign_payment && (
            <CustomSelect id="payment_type" formik={formik} label="Payment Type" placeholder="Select Payment Type">
              ...
              <Material.MenuItem value="Payment after campaign">Payment After Campaign</Material.MenuItem>
            </CustomSelect>
          )}
